 ...
 * /squeeze map      — read-only: interactive HTML checkpoint map
 *
 ...
 *   summarizerReasoningEffort — effort pin for the summarizer route; null (default)
 *                              sends unpinned. Pin e.g. 'low' only if the route's
 *                              effort map declares it
...
const DEFAULT_SUMMARIZER_EFFORT = null
...
/**
 * Session event log accessor across API generations: 0.1.2 renamed the eager
 * `events` array to `snapshotEvents()` (frozen snapshot, same 0-based order).
 */
function eventsOf(session) {
  return Array.isArray(session.events) ? session.events : session.snapshotEvents()
}
...
function estimateSurfaceTokens(session, tokenMeter) {
  ...
  for (const seq of session.surface.nodes) {
    const message = messageOf(eventsOf(session)[seq])
...
function findOpenTurn(session) {
  for (let i = eventsOf(session).length - 1; i >= 0; i--) {
    const evt = eventsOf(session)[i]
...
function isCompactionActive(session) {
  for (let i = eventsOf(session).length - 1; i >= 0; i--) {
    ...
    // An unmatched start older than the newest end-seed is stale evidence
    // from a prior process lifecycle: the compaction engine treats it as
    // released, and so does this check (a crash mid-compaction must not
    // wedge the command forever).
    if (evt.type === 'session/end-seed') return false
  }
  return false
}

// ── Balanced-edge snapping ────────────────────────────────────────────────────────────
...
const SPAN_SUMMARY_PROMPT = 'You are a compaction engine for an AI coding assistant. Write a high-fidelity summary of the conversation span below that lets the assistant resume work with no loss of essential context. Preserve verbatim: file paths, function and identifier names, commands, error messages, user decisions, and key findings. Discard verbose output, raw data dumps, and repetition. Write dense factual prose, no preamble. End with a final line "Span end-state:" listing every task this span touched as COMPLETED (with outcome) or STILL PENDING — never leave a task\'s status ambiguous.'

const OPEN_WORK_DIGEST_PROMPT = 'You read the most recent uncompressed messages of an AI coding assistant\'s conversation, taken immediately after a context compaction replaced older history with summaries. Output ONLY the work that is still open as of the FINAL message: either the single word "none" or a short bullet list (one line each, verb-first). A "next steps" or todo list in any message except the final assistant message is STALE history — do not list it. Do not list completed work.'
...
async function summarizeSpan(llm, provider, model, spanText, maxSummaryTokens, summarizerEffort, session, signal) {
  return summarizeWithPrompt(llm, provider, model, SPAN_SUMMARY_PROMPT, spanText, maxSummaryTokens, summarizerEffort, session, signal)
}

/**
 * Post-squeeze open-work digest: one cheap-route call over the uncompressed
 * tail, restating what is still open as of the final message. The checkpoint
 * summaries describe historical work without completion status, so the model
 * can mistake a finished task for pending work and re-enter it; this trusted
 * restatement in the tool result outranks every checkpoint summary. Fails
 * soft — the caller omits the block on any error.
 */
async function digestOpenWork(llm, options, session, signal) {
  const tailSeqs = session.surface.nodes.slice(-8)
  const tailText = tailSeqs
    .map((seq) => extractMessageText(eventsOf(session)[seq]))
    .filter((text) => text.length > 0)
    .join('\n\n')
  if (tailText.trim().length === 0) return { ok: false, error: 'empty tail' }
  return summarizeWithPrompt(
    llm, options.summarizerProvider, options.summarizerModel, OPEN_WORK_DIGEST_PROMPT,
    tailText, 200, options.summarizerReasoningEffort, session, signal,
  )
}

async function summarizeWithPrompt(llm, provider, model, systemPrompt, spanText, maxSummaryTokens, summarizerEffort, session, signal) {
  let summaryText = ''
  try {
    for await (const chunk of llm.stream({
      ...
      if (chunk.type === 'text-delta' && typeof chunk.text === 'string') {
        ...
        if (reason !== undefined && reason.kind === 'error') {
          return { ok: false, error: `summarizer stream finished with error${reason.message !== undefined ? ': ' + String(reason.message).slice(0, 200) : ''}` }
        }
 ...
 * cuts, summarize spans on the cheap route (bounded parallelism, NO lock
 * held), then commit each span through its own tight synchronous compaction
 * bracket. One bracket at a time honors the compaction capability's
 * single-lock contract; the long summarizer awaits happen bracketless, so a
 * crash or abort mid-summarization leaves no dangling lock.
...
async function executeContextSqueeze(args, exec, ctx, squeezeModes, options) {
  ...
  // earlier one after trimming.
  const plannedSpans = []

  // ── Phase 1: validate + trim (synchronous, no events appended) ──────────
  const pending = []
  for (const squeeze of sorted) {
    try {
      ...
      // Trim this range to the free sub-interval: head-clipping intruders
      // (their end inside this range) raise the floor; tail-clipping
      // intruders (their start inside this range) lower the ceiling. A span
      // fully covered by one intruder, or split by one, drops with an honest
      // message.
      const intruders = plannedSpans.filter((s) => startIdx <= s.endIdx && endIdx >= s.startIdx)
      const fullyCovered = intruders.some((s) => s.startIdx <= startIdx && s.endIdx >= endIdx)
      if (fullyCovered) {
      ...
      const floorCands = intruders.filter((s) => s.endIdx >= startIdx && s.endIdx < endIdx).map((s) => s.endIdx + 1)
      const ceilCands = intruders.filter((s) => s.startIdx > startIdx && s.startIdx <= endIdx).map((s) => s.startIdx - 1)
      const floor = floorCands.length > 0 ? Math.max(...floorCands) : -1
      const ceiling = ceilCands.length > 0 ? Math.min(...ceilCands) : nodes.length
      const wasStart = startIdx
      const wasEnd = endIdx
      startIdx = Math.max(startIdx, floor)
      endIdx = Math.min(endIdx, ceiling)
      if (startIdx > endIdx) {
        failures.push(`Range ${squeeze.start}-${squeeze.end}: split by a neighboring span that expanded to balanced edges; squeeze each side in separate calls.`)
        continue
      }
      if (startIdx !== wasStart || endIdx !== wasEnd) {
        adjustments.push(`Span ${squeeze.start}-${squeeze.end} trimmed to ${startIdx + 1}-${endIdx + 1} to fit neighboring spans (was ${wasStart + 1}-${wasEnd + 1}).`)
      }
      ...
      pending.push({ squeeze, startIdx, endIdx, shadowedSeqs, shadowedTokenCount, spanText, summaryOutcome: null })
    } catch (error) {
      failures.push(`Range ${squeeze?.start}-${squeeze?.end}: validation failed: ${error instanceof Error ? error.message : String(error)}`)
    }
  }

  // ── Phase 2: bounded-parallel summarization, NO lock held ───────────────
  // The thunk must never throw: a rejection would strand the phase-3 loop.
  // summarizeSpan is total; the catch belt makes the contract structural.
  await runBounded(pending, options.summarizerConcurrency, async (task) => {
    try {
      ...
      task.summaryOutcome = { ok: false, error: `summarizer crashed: ${error instanceof Error ? error.message : String(error)}` }
    }
  })

  // ── Phase 3: sequential commits, one tight bracket per span ─────────────
  // Each bracket opens and closes inside one synchronous try, so no code
  // path — abort, summarizer failure, append rejection — can leave a lock
  // dangling. The turn is derived at commit time (the agent-loop awaits
  // tool promises before turn/end, and the compaction invariant rejects a
  // turn boundary crossing an open bracket).
  let succeeded = 0
  ...
  for (const task of pending) {
    ...
    if (task.summaryOutcome === null || !task.summaryOutcome.ok) {
      const detail = task.summaryOutcome === null ? 'no outcome recorded' : task.summaryOutcome.error
      failures.push(`Range ${rangeLabel}: summarizer failed (${detail}).`)
      continue
    }
    const summaryText = task.summaryOutcome.summary

    // Stability: if the surface moved during summarization, the span's nodes
    // may no longer all be live — refuse rather than replace blind. (The
    // session's replace validation also requires sourceEventSeqs to cover
    // every node between start and end, so a mid-summarization insertion
    // inside the span makes the replace append throw cleanly.)
    ...
    if (isCompactionActive(session)) {
      failures.push(`Range ${rangeLabel}: a compaction became active during summarization; the conversation is unchanged. Retry with fresh positions.`)
      continue
    }

    const lifecycle = { compactionId, turn: findOpenTurn(session), ...sourceCommandId === undefined ? {} : { sourceCommandId } }
    ...
    try {
      ...
      try {
        ...
        session.append('user/message', checkpointMessage, {
          ...
          sourceEventSeqs: [startEvent.seq, summaryEvent.seq, ...task.shadowedSeqs],
        })
        ...
        try {
          session.append('compaction/end', { ...lifecycle, error: `${innerError?.name ?? 'Error'}: ${message}` })
        } catch { /* the log itself rejected the close; nothing more to do */ }
        failures.push(`Range ${rangeLabel} failed: ${message}`)
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)
      failures.push(`Range ${rangeLabel} failed to open compaction: ${message}`)
    }
  }
  ...
  // Trusted current-state restatement: prevents the model re-entering work
  // that a checkpoint summary describes without its completion status.
  if (succeeded > 0) {
    const digest = await digestOpenWork(llm, options, session, signal).catch(() => null)
    if (digest !== null && digest.ok) {
      const openWork = digest.summary.trim()
      parts.push(`Work described in the squeezed spans above is historical. Current open work (from the uncompressed tail): ${openWork.toLowerCase() === 'none' ? 'none' : openWork}`)
    }
  }
...
const USAGE = 'Usage: /squeeze [NNk | NN | status | map | help] — manual budget-targeted compression (no arguments = toward contextBudgetTokens)'
...
const HELP_TEXT = [
  ...
  '                       (waves, before/after, per-squeeze surface snapshots;',
  '                       written to tmp and opened in your browser)',
...
function parseArgs(raw) {
  ...
  if (tokenKMatch !== null) {
    ...
    return { target: tokens, status: false, help: false, map: false, error: null }
  }
  if (trimmed.toLowerCase() === 'map') {
    return { target: null, status: false, help: false, map: true, error: null }
  }
  const tokenMatch = trimmed.match(/^(\d+)$/)
  if (tokenMatch !== null) {
    const tokens = parseInt(tokenMatch[1], 10)
    if (tokens < 1) return { target: null, status: false, map: false, error: 'Invalid token target. Usage: /squeeze NN (tokens) or NNk.' }
    return { target: tokens, status: false, help: false, map: false, error: null }
  }
  return { target: null, status: false, help: false, error: `Unknown argument "${trimmed}". ${USAGE}` }
}

import { buildMapData, renderMapHtml } from './map-page.js'
...
export function apply(ctx, config = {}) {
  ...
  ctx.commands.register({
    name: 'squeeze',
    description: 'Manual budget-targeted context compression; summaries written by a cheap route. Usage: /squeeze [NNk|NN|status|map|help]',
    input: { hint: '[NNk|NN|status|map|help]' },
    handler: async (invocation) => {
      const { target, status, help, map, error } = parseArgs(invocation.rawInput)
      ...
      if (map) {
        // Read-only visualization: replay the session log into an interactive
        // HTML map (checkpoints, before/after, per-pass surface snapshots).
        try {
          const events = eventsOf(session)
          const shortId = String(session.id).slice(0, 8)
          const data = buildMapData(events, 'Squeeze map — session ' + shortId)
          const html = renderMapHtml(data)
          const file = path.join(tmpdir(), 'squeeze-map-' + shortId + '.html')
          fs.writeFileSync(file, html)
          // Best-effort open; failure to launch a viewer is not an error.
          try { spawn('xdg-open', [file], { detached: true, stdio: 'ignore' }).unref() } catch { /* viewer optional */ }
          return { kind: 'success', text: 'Squeeze map written and opened: ' + file + ' — ' + data.brackets.length + ' checkpoints over ' + data.totalEvents + ' logged events. Read-only; nothing was changed.' }
        } catch (error) {
          return { kind: 'error', text: 'Failed to build squeeze map: ' + (error instanceof Error ? error.message : String(error)) }
        }
      }
