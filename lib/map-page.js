...
// ── squeeze waves: checkpoints clustered by commit-time gaps ────────────────
// One /squeeze invocation = a burst of commits; a gap over 10 minutes starts a
// new wave. Lanes, badges, and version snapshots are all derived from this.
const WAVE_GAP_MS = 10 * 60 * 1000
const commitOrder = [...brackets] // already sorted by commitSeq
const waves = []
for (const b of commitOrder) {
  const prev = waves[waves.length - 1]
  if (prev === undefined || b.time - prev.lastTime > WAVE_GAP_MS) waves.push({ firstTime: b.time, lastTime: b.time, count: 0 })
  const w = waves[waves.length - 1]
  w.count++; w.lastTime = b.time
  b.wave = waves.length - 1
}

// ── surface replay: what the agent saw after each squeeze wave ───────────────
const isMsg = (e) => ['user/message', 'assistant/message', 'tool/result'].includes(e?.type)
const msgByText = new Map(allMsgs.map((m) => [m.seq, m]))
const checkpointAt = new Map() // checkpoint message seq -> bracket index
const versions = [{ label: 'original — before any squeeze', seqs: null, _snap: null }]
let surface = [], k = 0, before = true
for (const e of events) {
  if (!isMsg(e)) continue
  ...
  if (e.type === 'user/message' && realOp?.op === 'replace') {
    ...
    const { start, end } = realOp
    surface = surface.filter((q) => q < start || q > end)
    surface.push(e.seq)
    const b = commitOrder[k]; k++
    if (b) checkpointAt.set(e.seq, commitOrder.indexOf(b))
    // snapshot at each wave boundary (last checkpoint of its wave) and at the very end
    const next = commitOrder[k]
    if (b && (next === undefined || next.wave !== b.wave)) {
      versions.push({ label: 'after squeeze ' + (b.wave + 1) + ' (' + new Date(waves[b.wave].firstTime).toLocaleDateString() + ')', seqs: null, _snap: [...surface] })
    }
    continue
  }
  if (msgByText.has(e.seq)) surface.push(e.seq)
}
if (before) versions[0]._snap = [...surface] // never squeezed: original == current
...
const data = { lastSeq, totalEvents: sourceEvents.length, brackets, buckets: counts, bucketSize: bs, allMsgs,
  ...
  waves: waves.map((w, i) => ({ label: 'squeeze ' + (i + 1), firstTime: w.firstTime, count: w.count })),
  title }
  return data
}
...
<div class="layout" id="layout"><div class="left">
 <h2>Log timeline (0 → <span class="mono" id="ls"></span> seqs) — one lane per /squeeze invocation; click a bar</h2>
 <div id="strip"></div>
 <div class="legend"><span>each lane = one /squeeze invocation (S1 oldest → S<span id="nwaves"></span> newest; colors repeat after 8)</span><span>unmarked = still live</span></div>
...
document.getElementById('nwaves').textContent=D.waves.length;
const strip = document.getElementById('strip');
const PAL=['#3b82f6','#ef4444','#14b8a6','#f59e0b','#a78bfa','#ec4899','#84cc16','#f97316'];
const laneH=Math.min(34,Math.max(20,Math.floor(380/Math.max(1,D.waves.length))));
strip.style.height=(D.waves.length*laneH+12)+'px';
D.waves.forEach((w,li)=>{
 const lane=document.createElement('div');lane.className='lane';lane.style.top=(li*100/D.waves.length)+'%';lane.style.height=(100/D.waves.length)+'%';
 const lb=document.createElement('label');lb.textContent='S'+(li+1);
 lb.title=w.label+' · '+new Date(w.firstTime).toLocaleString()+' · '+w.count+' checkpoints';lane.appendChild(lb);
 const box=document.createElement('div');box.className='lanebox';box.style.height=Math.max(12,laneH-6)+'px';lane.appendChild(box);
 strip.appendChild(lane);
 D.brackets.forEach((b,i)=>{if(b.wave!==li)return;const s=document.createElement('div');s.className='seg';
  s.style.left=(b.start/D.lastSeq*100)+'%';s.style.width=Math.max(0.35,(b.end-b.start)/D.lastSeq*100)+'%';
  s.style.height=Math.max(12,laneH-6)+'px';
  s.style.background=PAL[li%PAL.length];s.style.opacity='0.9';
  s.title=w.label+' · '+new Date(w.firstTime).toLocaleDateString()+' · seq '+b.start.toLocaleString()+'–'+b.end.toLocaleString()+' · '+b.n+' msgs → 1 ckpt';s.onclick=()=>sel(i);box.appendChild(s)});
});
...
D.brackets.forEach((b,i)=>{const tr=document.createElement('tr');const kept=b.tokens>0?Math.round(Math.min(100,b.sum.length*4/b.tokens*100)):0;
 tr.innerHTML='<td>'+(i+1)+'</td><td><span class="pass" style="background:'+PAL[b.wave%PAL.length]+'2e;color:'+PAL[b.wave%PAL.length]+'">S'+(b.wave+1)+'</span></td><td class="mono">'+fmt(b.start)+'–'+fmt(b.end)+'</td><td>'+b.n+'→1</td><td>~'+fmt(b.tokens)+'</td><td>'+fmt(b.sum.length)+' ch</td><td>'+kept+'%</td>';
...
function sel(i){setTimeout(()=>{const orig=document.getElementById('orig');if(!orig)return;
 ...
 document.getElementById('detail').innerHTML='<h2>Checkpoint '+(i+1)+' <span class="pass" style="background:'+PAL[b.wave%PAL.length]+'2e;color:'+PAL[b.wave%PAL.length]+'">squeeze '+(b.wave+1)+'</span>'+(surfaceIdx?' <span style="color:#7c8db0;font-size:11px">· surface item #'+surfaceIdx+' ('+D.versions[curV].label+')</span>':'')+'</h2>'+
...
const ckEl=(seq,idx)=>{const gi=D.ckAt[seq];const b=D.brackets[gi];
 ...
 d.style.borderLeft='3px solid '+PAL[gi%PAL.length];d.style.paddingLeft='8px';
 ...
 sm.textContent='#'+idx+' · ◆ CHECKPOINT '+(gi+1)+' (squeeze '+(b.wave+1)+') · stands in for '+b.n+' original messages · ~'+fmt(b.tokens)+' tok';
 d.appendChild(sm);
